...
pub fn extract_ai_markers(file_path: &RelativePath, content: &str) -> Vec<ManifestCustomizeMarker> {
...
fn parse_ai_marker(line: &str) -> Option<String> {
    ...
    let hint_value_start = hint_start + 6;
...
mod tests {
    ...
    save(validated)
        ...
        let markers = extract_ai_markers(&path("src/orders/service.rs"), content);
        assert_eq!(markers.len(), 1);
        assert_eq!(markers[0].path.as_ref(), "src/orders/service.rs");
    ...
    fn extract_python_markers() {
        ...
        let markers = extract_ai_markers(&path("app/service.py"), content);
        assert_eq!(markers.len(), 1);
    ...
    fn extract_html_markers() {
        ...
        let markers = extract_ai_markers(&path("index.html"), content);
        assert_eq!(markers.len(), 1);
    ...
    fn extract_multiple_markers() {
        ...
        let markers = extract_ai_markers(&path("file.rs"), content);
    ...
    fn no_markers() {
        let content = "fn main() {\n    println!(\"hello\");\n}";
        let markers = extract_ai_markers(&path("main.rs"), content);
